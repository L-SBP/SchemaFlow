...
SPIDER_TRAIN_PATH = "../train_spider.json"
# SPIDER_OTHERS_PATH = "../train_others.json"

# 输出文件
OUTPUT_FILE = "./spider_dml_extended.json"

# AI 服务配置
# 【修复点】URL 必须包含 /v1/chat/completions
# AI_SERVICE_URL = "https://api-inference.modelscope.cn/v1/chat/completions"
# AI_API_KEY = "ms-5990c475-35aa-444d-80eb-01c7f0d5719b"
...
def get_train_db_ids():
    """获取所有训练集涉及的数据库 ID (白名单)"""
    db_ids = set()
    
    # 1. 读取 train_spider.json
    ...
    # # 2. 读取 train_others.json
    # if os.path.exists(SPIDER_OTHERS_PATH):
    #     with open(SPIDER_OTHERS_PATH, 'r', encoding='utf-8') as f:
    #         data = json.load(f)
    #         for item in data:
    #             db_ids.add(item['db_id'])
                
    print(f"Found {len(db_ids)} unique databases in training sets.")
    return db_ids
...
async def main():
    ...
    # 3. 过滤：只保留训练集里的数据库
    target_dbs = [db for db in all_tables if db['db_id'] in train_db_ids]
    print(f"Generating DML data for {len(target_dbs)} databases (filtered from {len(all_tables)} total)...")
