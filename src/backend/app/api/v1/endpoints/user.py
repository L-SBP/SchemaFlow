...
# ----------------------------------------------------------------------
@router.get("/me", response_model=UnifiedResponse[schemas.UserMe])
async def read_user_me(
    ...
    Returns:
        UnifiedResponse[UserMe]: 用户信息。

    Raises:
        HTTPException: 用户未找到(404)或内部服务器错误(500)。
    """
    # Service 层负责将 ORM 转换为 UserMe DTO
    result = await user_service.get_user_me_service(db, current_user.user_id)
    return UnifiedResponse.success(data=result, message="获取用户信息成功")


# ----------------------------------------------------------------------
# 2. PATCH /users/me - 更新用户名 (Doc 3.1.2)
# ----------------------------------------------------------------------
@router.patch("/me", response_model=UnifiedResponse[schemas.UserMe])
async def update_username_endpoint(
    ...
    Returns:
        UnifiedResponse[UserMe]: 更新后的用户信息。
    ...
    return UnifiedResponse.success(data=updated_user_dto, message="用户名更新成功")
...
async def update_password_endpoint(
    ...
    # 成功返回统一响应
    return UnifiedResponse.success(message="密码更新成功")


# ----------------------------------------------------------------------
# 4. POST /users/me/avatar - 上传/更换头像 (Doc 3.1.6)
# ----------------------------------------------------------------------
@router.post("/me/avatar", response_model=UnifiedResponse[schemas.UserUpdateAvatar])
async def update_avatar_endpoint(
    ...
    Returns:
        UnifiedResponse[UserUpdateAvatar]: 更新后的头像信息。
    ...
    return UnifiedResponse.success(data=updated_avatar_dto, message="头像更新成功")

# ----------------------------------------------------------------------
# 5. POST /users/me/email/send-code - 请求更新邮箱 (Doc 3.1.4)
# ----------------------------------------------------------------------
@router.post("/me/email/send-code", response_model=UnifiedResponse[None])
...
# ----------------------------------------------------------------------
@router.put("/me/email", response_model=UnifiedResponse[schemas.UserMe])
async def confirm_update_email_endpoint(
    ...
    return UnifiedResponse.success(data=updated_user_dto, message="邮箱更新成功")


@router.get("/me/login-history", response_model=UnifiedResponse[PageData[list]])
async def read_login_history(
    ...
    Returns:
        UnifiedResponse[PageData[list]]: 分页登录历史记录。
    """
    ...
    # 将原有分页结构转换为新的 PageData 结构
    page_data = PageData(
        total=history_list.total,
        page=history_list.page,
        page_size=history_list.page_size,
        items=history_list.items
    )
    return UnifiedResponse.success(data=page_data, message="获取登录历史成功")
