...
class MySQLConverter:
    """
    SQLite到MySQL深度转换器 (完全兼容 sqlglot 24.0+)
    专为模式转换优化，精准处理保留字/主键/精度/外键/字符集等关键问题
    """

    # MySQL保留字集合 (来自MySQL 8.0官方文档)
    ...
    FIELD_PRECISION_RULES = {
        ...
        # 数量/计数字段
        r"(?i)(quantity|count|num|stock|capacity)": ("INT", None),
        # 折扣率/百分比字段
    ...
    def _transform_create_table(self, node):
        ...
        if table_name and table_name in self.MYSQL_RESERVED_WORDS:
            # 用反引号包裹表名
    ...
    def _add_table_properties(self, node):
        ...
        if properties and hasattr(properties, "expressions"):
            for prop in properties.expressions:
                if hasattr(prop, "this") and str(prop.this).upper() == "ENGINE":
                    has_engine = True
        ...
        # 创建属性列表
        new_properties = [
            PropertyExpr(this="ENGINE", value=LiteralExpr.string("InnoDB")),
            PropertyExpr(this="DEFAULT CHARACTER SET", value=LiteralExpr.string("utf8mb4")),
            PropertyExpr(this="COLLATE", value=LiteralExpr.string("utf8mb4_unicode_ci")),
        ]
        
        # 创建Properties节点
        prop_expr = PropertiesExpr(expressions=new_properties)
        
        # 设置属性 (兼容新旧结构)
        if hasattr(node, "set"):
            node.set("properties", prop_expr)
        else:
            node.args["properties"] = prop_expr
    ...
    def convert_schema(self, sqlite_sql: str) -> str:
        ...
        try:
            ...
            mysql_sql = self.generic_converter.convert(
                sqlite_sql, 'sqlite', 'mysql', pretty=True
            )
            ...
            if self.deep_conversion_enabled:
                try:
                    # 关键修复: 使用正确的read方言
                    ast = parse_one(mysql_sql, read="mysql", dialect="mysql")
                    self._apply_ast_transformations(ast)
                    mysql_sql = ast.sql(dialect="mysql", pretty=True)
                except Exception as ast_err:
                    self.conversion_stats["ast_conversion_failures"] += 1
                    print(f"AST转换失败，回退到基础转换: {str(ast_err)}")
            
            # 兜底修复
            mysql_sql = self._apply_fallback_fixes(mysql_sql)
            return mysql_sql
        except Exception as e:
            raise SQLConversionError(f"SQLite到MySQL模式转换失败: {str(e)}")
    ...
    def _apply_fallback_fixes(self, sql: str) -> str:
        ...
        # 3. 添加表选项 (如果缺失)
        if not re.search(r'ENGINE\s*=', sql, re.IGNORECASE):
            sql = re.sub(
                r'\)\s*;',
    ...
    def convert_statement(self, sqlite_sql: str) -> str:
        ...
        try:
            # 基础转换
            mysql_sql = self.generic_converter.convert(
                sqlite_sql, 'sqlite', 'mysql', pretty=False
            )

            # 仅对DDL语句进行深度转换
            if any(keyword in mysql_sql.upper() for keyword in ["CREATE TABLE", "ALTER TABLE"]):
                return self.convert_schema(mysql_sql)  # 复用schema转换

            # DML/其他语句 - 应用轻量级修复
            return self._apply_statement_fixes(mysql_sql)
        except Exception as e:
            raise SQLConversionError(f"SQLite到MySQL语句转换失败: {str(e)}")
...
CREATE TABLE User_Booking (UserID TEXT NOT NULL, BookingID TEXT NOT NULL, PaymentMethod TEXT, LoyaltyProgram TEXT, PRIMARY KEY(UserID, BookingID), FOREIGN KEY(UserID) REFERENCES User(UserID), FOREIGN KEY(BookingID) REFERENCES Booking(BookingID));"""
    ...
    for sql in sqlite_create_table.split(';'):
        try:
            mysql_create_table = converter.convert_schema(sql)
            print("SQLite:")
            print(sql)
            print("\n转换后的MySQL:")
